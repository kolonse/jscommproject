...
module.exports = function(app) {
    ...
    function processReq(param, cb) {
        ...
        if (!data.extra) {
            ...
            app.logger.error("%s 解禁 %j 操作失败,未选中操作数据", param.fcadmin, data);
            return;
        }
        var id = parseInt(data.extra.id);
        // data.password = app.Common.DEFAULT_PASSWARD;
        // 对数据库进行插入操作
        app.dbhelper.updateProxy({ status: app.Common.PLAYER_STATUS.unfreeze }, { id: id }, function(err) {
            if (err) {
                res.code = app.ErrorCode.errorDBBase;
                res.message = err.message;
                process.nextTick(function() { cb(res); });
                app.logger.error("%s 解禁 %j 数据异常,err:", param.fcadmin, data, err.stack);
                return;
            }
            process.nextTick(function() { cb(res); });
            app.logger.info("%s 解禁 %j 数据成功", param.fcadmin, data);
            return;
        })
    }
}