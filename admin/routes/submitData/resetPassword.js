...
module.exports = function(app) {
    ...
    function processReq(param, cb) {
        ...
        // 对数据库进行插入操作
        app.dbhelper.updateProxy({ password: app.Common.DEFAULT_PASSWARD }, { id: id }, function(err) {
            if (err) {
                res.code = app.ErrorCode.errorDBBase;
                res.message = err.message;
                process.nextTick(function() { cb(res); });
                console.log(err);
                app.logger.info("%s 重置密码 %j 数据异常,err:", param.fcadmin, data, err.stack);
                return;
            }
            process.nextTick(function() { cb(res); });
            app.logger.info("%s 重置密码 %j 数据成功", param.fcadmin, data);
            return;
        });
    }
};