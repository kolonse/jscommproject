...
module.exports = function(app) {
    ...
    function processReq(param, cb) {
        ...
        if (!data.extra) {
            ...
            app.logger.error("%s 增加工会 %j 操作失败,未选中操作数据", param.fcadmin, data);
            return;
        }
        ...
        app.dbhelper.updateProxy({
            ...
            if (err) {
                ...
                process.nextTick(function() { cb(res); });
                app.logger.error("%s 增加工会 %j 数据异常,err:", param.fcadmin, data, err.stack);
                return;
            }
            process.nextTick(function() { cb(res); });
            app.logger.info("%s 增加工会 %j 数据成功", param.fcadmin, data);
            return;
        })
    }
}