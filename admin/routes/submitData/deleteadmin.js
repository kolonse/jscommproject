    ...
    app.post("/submitData/" + thisId, function(req, res) {
        ...
        function processReq(param, cb) {
            ...
            if (!data.extra) {
                ...
                app.logger.error("%s 删除管理员 %j 操作失败,未选中操作数据", param.fcadmin, data);
                return;
            }
            ...
            if (isNaN(id)) {
                ...
                process.nextTick(function() { cb(res); });
                app.logger.error("%s 删除管理员 %j 操作失败,用户 id 非合法数字", param.fcadmin, data);
                return;
            }
            app.dbhelper.delete("user", { id: id }, function(err) {
                if (err) {
                    res.code = app.ErrorCode.errorDBBase;
                    res.message = err.message;
                    process.nextTick(function() { cb(res); });
                    app.logger.error("%s 删除管理员 %j 数据异常,err:", param.fcadmin, data, err.stack);
                    return;
                }
                process.nextTick(function() { cb(res); });
                app.logger.info("%s 删除管理员 操作成功", param.fcadmin);
            });
        }
    });
};