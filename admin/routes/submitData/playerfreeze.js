...
module.exports = function(app) {
    ...
    function processReq(param, cb) {
        ...
        if (!data.extra) {
            ...
            app.logger.error("%s 冻结玩家 %j 操作失败,未选中操作数据", param.fcadmin, data);
            return;
        }
        ...
        var url = param.currentGameAppUrl + "freeze";
        var http = Http();
        http.post(url, { uid: uid }, function(err, result) {
            if (err) {
                ...
                process.nextTick(function() { cb(res); });
                app.logger.error("%s 冻结玩家 %s 操作失败,游戏服务器回调接口 %s 异常,err:", param.fcadmin, uid, url, err.stack);
                return;
            }
            if (result.code === undefined) {
                res.code = app.ErrorCode.errorRemoteCall;
                res.message = result;
                app.logger.error("%s 冻结玩家 %s 操作失败,游戏服务器回调接口 %s 失败,result:%j", param.fcadmin, uid, url, result);
                process.nextTick(function() { cb(res); });
                return;
            }
            if (result.code !== 0) {
            ...
            process.nextTick(function() { cb(res); });
            app.logger.info("%s 冻结玩家 %j 操作成功", param.fcadmin, data);
        });
    }
};